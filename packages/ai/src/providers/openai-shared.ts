...
function hasResponsesCacheBreakpoint(input: InputItem[] | undefined): boolean {
	return (
		input?.some(
			message =>
...
const GPT_5_6_FLOOR: Revision = [5, 6, 0];

/**
 * GPT-5.6+ family check shared by Responses routes.
 *
 * The model id classifies the reasoning family regardless of the provider or
 * host serving it, so compatible gateways get the same behavior.
 */
export function isGpt56PlusResponsesModel(model: Model<Api>): boolean {
	const identity = classifyModel("", bareModelId(model.requestModelId ?? model.id), { lenient: true });
	if (identity.class !== "openai" || identity.revision === undefined) return false;
	const revision = parseRevision(identity.revision);
	return revision !== undefined && compareRevision(revision, GPT_5_6_FLOOR) >= 0;
}

export interface OpenAIResponsesPromptCacheParams {
	input?: InputItem[];
	prompt_cache_options?: { mode: "implicit" | "explicit"; ttl?: "30m" } | null;
}

export function applyOpenAIResponsesPromptCachePolicy(
	params: OpenAIResponsesPromptCacheParams,
	model: Model<"openai-responses"> | Model<"openai-codex-responses">,
	options: Pick<StreamOptions, "cacheRetention" | "promptCache"> | undefined,
