...
import {
	...
	type SubagentIrcRoster,
} from "@oh-my-pi/pi-coding-agent/task/subagent-system-prompt";
...
describe("subagent system prompt templates", () => {
	...
	test.serial("reads the current IRC roster when the prompt transform runs", async () => {
		await withTemplateEnvironment(async ({ projectDir }) => {
			let roster: SubagentIrcRoster = {
				peers: [{ id: "early", displayName: "Early", kind: "task", status: "running" }],
				parkedCount: 0,
				omittedCount: 0,
			};
			const resolved = await resolveSubagentSystemPrompt(projectDir, {
				agent: "Rendered scout role",
				ircSelfId: "Child",
				ircRoster: () => roster,
			});
			roster = {
				peers: [{ id: "late", displayName: "Late", kind: "task", status: "idle" }],
				parkedCount: 2,
				omittedCount: 1,
			};

			const result = resolved.transform({
				systemPrompt: ["base-block", "project-block"],
				dynamicParts: [],
				xdevCatalogNames: [],
			});

			expect(result.systemPrompt[1]).toContain("`late` — Late (task, idle)");
			expect(result.systemPrompt[1]).toContain("1 more live peer(s) omitted.");
			expect(result.systemPrompt[1]).toContain("2 parked peer(s) omitted.");
			expect(result.systemPrompt[1]).not.toContain("`early`");
		});
	});
