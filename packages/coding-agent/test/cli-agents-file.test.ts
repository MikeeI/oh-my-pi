...
describe("parseArgs — process-scoped prompt file flags", () => {
	...
	it("does not recognize the upstream template flag as an alias", () => {
		const parsed = parseArgs(["--system-prompt-template", "legacy.md", "hello"]);

		expect(parsed.systemTemplate).toBeUndefined();
		expect(parsed.unrecognizedFlags).toContain("--system-prompt-template");
	});
