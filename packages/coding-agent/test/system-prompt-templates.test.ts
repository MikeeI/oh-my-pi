...
describe("system Handlebars prompt templates", () => {
	...
	test("empty selected system prompt template hard fails", async () => {
		await withTempDir(async dir => {
			...
			await expect(buildCapturedTemplatePrompt(dir, options)).rejects.toThrow("must not be empty");
		});
	});

	test("missing selected system prompt template hard fails", async () => {
		await withTempDir(async dir => {
			const templatePath = path.join(dir, ".omp", "SYSTEM.template.md");
			await fs.mkdir(path.dirname(templatePath), { recursive: true });
			await fs.writeFile(templatePath, "Selected template");

			const { options } = await captureRootCommandOptions(dir);
			await fs.rm(templatePath);

			await expect(buildCapturedTemplatePrompt(dir, options)).rejects.toThrow(
				`Could not read system prompt template file: ${templatePath}`,
			);
		});
	});

	test("system-prompt inspect uses process-scoped template and AGENTS overrides", async () => {
		await withTempDir(async dir => {
			const discoveredTemplatePath = path.join(dir, ".omp", "SYSTEM.template.md");
			const explicitTemplatePath = path.join(dir, "SYSTEM.explicit.template.md");
			const agentsPath = path.join(dir, "AGENTS.explicit.md");
			await fs.mkdir(path.dirname(discoveredTemplatePath), { recursive: true });
			await fs.writeFile(discoveredTemplatePath, "Discovered template");
			await fs.writeFile(explicitTemplatePath, "Explicit {{cwd}}");
			await fs.writeFile(agentsPath, "Alternate global agent contract");

			const inspected = await inspectSystemPrompt(dir, {
				systemPromptTemplateSource: path.basename(explicitTemplatePath),
				userAgentsFile: path.basename(agentsPath),
			});
			const providerPrompt = inspected.systemPrompt.join("\n");

			expect(inspected.systemPrompt[0]).toContain("Explicit ");
			expect(inspected.systemPrompt[0]).not.toContain("{{cwd}}");
			expect(providerPrompt).not.toContain("Discovered template");
			expect(providerPrompt).toContain("Alternate global agent contract");
			expect(providerPrompt).toContain(`path="${agentsPath}"`);
		});
	});
	...
	test("buildSystemPrompt keeps system and project as separate ordered blocks", async () => {
		await withTempDir(async dir => {
			const nativeTemplate = await Bun.file(path.join(systemPromptsDir, "system-prompt.md")).text();
			const { systemPrompt } = await buildSystemPrompt({
				cwd: dir,
			...
			expect(systemPrompt[0]).toContain("<conventions>");
			...
			expect(systemPrompt[1].indexOf("</workstation>")).toBeLessThan(systemPrompt[1].indexOf("<workspace-tree>"));
		});
	});
	...
	test("system prompt template renders with the native prompt data model", async () => {
		const { systemPrompt } = await buildSystemPrompt({
			...
			tools: new Map([
				["read", { label: "Read", description: "Reads files", readsSkillUris: true }],
	...
	test("buildSystemPrompt renders workspace tree after directory context in project prompt", async () => {
		await withTempDir(async dir => {
			...
			expect(projectPrompt).toContain("Working-directory layout: newest mtime first; depth ≤ 3.");
			expect(projectPrompt).toContain("Some entries elided to shorten tree");
