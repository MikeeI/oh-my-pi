...
Every call MUST set `op`; `i` is only the concise intent label and never replaces `op`.
...
- `wait` returns on the first message, watched completion, adaptive wait-window timeout, or steering interrupt.
- Repeated back-to-back waits lengthen the window from 5 seconds up to 5 minutes; stepping away resets it.
