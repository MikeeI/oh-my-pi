...
export class TranscriptContainer extends Container {
	...
	#renderEmergency(
		...
		owners.push(newestActive.entry.component);
	...
	#renderActiveEmergency(
		...
		for (let attempt = 0; attempt < 2; attempt++) {
			...
			// The summary row itself represents the newest block when no block row
			// fits beside it; report only the additional backlog.
			if (hidden === candidates.length && hidden > 0) hidden--;
