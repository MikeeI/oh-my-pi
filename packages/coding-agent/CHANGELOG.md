...
- Added profile-scoped `read.md` and `bash.md` overrides for experimenting with tool guidance without rebuilding MOMP.
...
- Added `bun run bench:hub-routing` to measure exact Hub call routing across five Codex GPT models.
- Added `momp stats --summary` multi-range usage rendering that remains available in global MOMP installations.
- Added a reproducible local conversation-search benchmark with fixed inputs and corpus-stability guardrails.
- Added Main-only `conversation_search` for bounded lexical lookup across persisted user and Assistant text.
...
- Hub guidance, schema hints, and examples reduce repetition while clarifying peer waits and process defaults.
- `read skill://` calls now use the compact grouped Read view while remaining expandable.
- `momp stats --summary` now shows mobile-safe rolling 24-hour, 7-day, and 30-day usage blocks.
- Stats summaries now show detailed agent, model, and folder consumption.
...
- Fixed Eval guidance implying that `agent()` children share parent kernel state.
