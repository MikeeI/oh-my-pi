...
- Reason: current upstream has no Main prompt-template discovery, strict template precedence, or process-scoped template workflow.
...
- Owner: `src/task/subagent-runtime-config.ts` owns shared Child settings and capability resolution.
...
- Reason: current upstream has no process-scoped `--system-template` or `--agents-file` capability.
...
- Owner: `src/task/subagent-runtime-config.ts` owns the inspected Child's settings and capability resolution.
...
- Owner: `packages/tui/src/chat/read-tool-group.ts#readArgsCollapseIntoGroup` owns the collapse decision.
