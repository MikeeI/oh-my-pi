...
- Disposition: `MOMP-EIGEN`.
- Contract: `prompts/tools/read.md`, `bash.md`, `task.md`, `lsp.md`, `eval.md`, and `web-search.md` under the active user agent directory override their bundled tool descriptions.
...
- Required action: retain model-policy runtime deltas, keep the bundled Task prompt byte-identical to upstream, and use the settings override for MOMP-specific effort guidance.
...
- Required action: retain only the skill protocol's compact-transcript spec delta.
- Proof: `test/read-tool-group.test.ts`.

#### `MOMP-READ-SCHEDULING` — Provider-roundtrip-efficient reads

- Disposition: `UPSTREAM-INTEGRIERT`.
- Contract: the bundled Read prompt stays byte-identical to upstream; the active profile guides parallel independent reads through `MOMP-TOOL-PROMPT-OVERRIDES`.
...
- Reason: current upstream lacks transcript-owned rename generation and command-owned argument-completion semantics.
...
#### `MOMP-CODEX-GPT6` — Current Codex GPT-6 discovery and pricing

- Disposition: `UPSTREAM-INTEGRIERT`.
- Contract: Codex discovery and request identities use client version `0.155.1`.
- Contract: refreshed account discovery exposes `gpt-6-sol` and `gpt-6-luna`.
- Contract: OpenAI API routes carry the documented Sol and Luna base and long-context tariffs.
- Contract: Codex routes carry Work/Codex token-equivalent rates, free cache writes, and the 2.5x fast tier.
- Contract: Sol and Luna requests above 272K prompt input use 2x input/cache and 1.5x output rates.
- Contract: Astra remains exempt from the additional Codex long-context multiplier.
- Contract: settings use model overrides only and never duplicate account-discovered models as manual entries.
- Owner: upstream `packages/catalog/src/wire/codex.ts#CODEX_CLIENT_VERSION` owns the shared identity.
- Owner: upstream provider KDL rules own OpenAI API and Codex pricing metadata.
- Reason: upstream v18.3.0 owns client identity `0.155.1`, account-scoped discovery, and Codex Sol/Luna base rates.
- Reason: its provider rules lack Codex Sol/Luna long-context and 2.5x priority rates and OpenAI API Sol/Luna base and long-context rates.
- Required action: retain only those missing pricing deltas at the provider KDL owners.
- Proof: GPT-6 pricing cases in `packages/catalog/test/generated-policies.test.ts`.
- Proof: plain and worker pricing in `packages/catalog/test/codex-discovery.test.ts`.
- Proof: candidate `models refresh --json` lists both models and real Codex print calls succeed for each.
...
- Reason: upstream v18.3.0 offers a simple `stats --summary` but not MOMP's package-local 24h/7d/30d output contract.
...
- Owner: `src/task/settings.ts#cfgTaskEnableLsp` owns the MOMP default and operator-facing setting.
...
- Reason: current upstream's architect prompt assumes `CLAUDE.md` instead of consuming supplied `AGENTS.md` context.
...
- Proof: full-result paths in `packages/tui/test/read-render.test.ts`.
