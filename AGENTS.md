...
- Owner: `AssistantMessageComponent#renderTranscriptBlockEmergencyRows` owns finalized emergency-tail row selection.
- Owner: upstream `FinalizableBlock#getTranscriptBlockVersion` owns post-finalize presentation versioning.
- Owner: `src/modes/utils/terminal-row-reflow.ts` owns accepted-row hard reflow when the replay width grows.
